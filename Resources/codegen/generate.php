...
foreach (array(true,false) as $interface) {
  ...
  foreach ($dirs as $dir) {
    ...
    echo "if (\$extension->getVersion()=='$matches[1]') {\n";  
    ...
    foreach ($lines as $line) {
      ...
      if ($interface) echo str_replace(' {}', ';', $line);
      else {
        preg_match('/public function ([^\(]*)\(/', $line, $matches);
        $function = $matches[1];
        $line = str_replace(' {}', ' {', $line);
        if ($function == '__construct') $php = $line.<<<END_OF_PHP
            \$this->calls = array();
...
END_OF_PHP;
  ...
  else echo file_put_contents('../../Cache/LoggingMemcache.php',ob_get_clean());
}